				...
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						<u-badge class="u-tab-text" :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></u-badge>
						{{ item[name] || item['name']}}
					</view>
			...
			tabItemStyle() {
				return (index) => {
					let style = {
						...
						'letter-spacing': this.letterSpacing + 'rpx'
					};
					// 字体加粗
					if (index == this.currentIndex && this.bold) style.fontWeight = 'bold';
					// 字体变大
					if (index == this.currentIndex) style.fontSize = this.fSize + 'px';
					if (index == this.currentIndex) {
						// style.color = this.activeColor;
						style.color = "#000"; // 选中色为黑色，而不是跟滑块一样的自定义传入颜色
