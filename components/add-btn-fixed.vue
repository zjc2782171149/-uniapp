<template>
	<view class="slot" @touchmove="buttonMove" @touchstart="buttonStart" @touchend="buttonEnd" :style="{ top: buttonTop + 'px', left: buttonLeft + 'px' }">
		<view class="btn" @click="click"><u-icon name="plus" size="46"></u-icon></view>
	</view>
</template>
	...
	mounted() {
		...
		uni.getSystemInfo({
			success(res) {
				...
				//这里定义按钮的初始位置
				$this.buttonTop = res.windowHeight * 0.89;
				//这里定义按钮的初始位置
				$this.buttonLeft = res.windowWidth * 0.805;
			}
		});
	},
	methods: {
...
.slot {
	...
	.btn {
		...
		background: url("/static/find/add.png");
