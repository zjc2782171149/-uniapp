<template>
	<view class="page">
		<!-- 带背景的透明导航栏 -->
		<NavbarRoundImg title="我的数据" isBack :bgImg="bgImg" :backgroundColor="backgroundColor"></NavbarRoundImg>
		<!-- 信息 -->
		<view class="info">
			<view class="avatar">
				<u-avatar size="132" :src="userInfo.icon ? userInfo.icon : '../../../static/user/1.png'"></u-avatar>
			</view>
...
<!-- 				<u-button type="gold" size="small" shape="circle">
		...
		<view class="item" v-if="currentTab == 0">
			<view class="card">
				...
				<view class="line">个人简介：{{ userInfo.personal_signature || '这个人很懒,没留下什么'}}</view>
				<view class="line">生日：{{ userInfo.birthday || '生日当然要保密啦'}}</view>
			</view>
