<template>
  ...
  >
    <view class="info">
      <view class="pic"
        ><u-image :src="data.pics" width="190rpx" height="190rpx"></u-image
      ></view>
      <view class="desc">
        <view class="price">
          <text>￥</text>
    ...
    <view class="sku">
      ...
      <view class="list">
        <view
          class="item"
          v-for="(item, index) in data.skuData"
          :key="index"
          :class="{ select: item.value === selectSkuValue }"
          @click="selectSku(item, index)"
        >
          {{ item.label }}
        </view>
      </view>
