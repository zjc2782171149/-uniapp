	...
	<view class="slot" @click.stop="goOrderInfo">
		...
		<OrderGoodsCard :data="data" showBorderBottom>
			<view class="status-slot">
				...
				<text v-if="data.status == 3 && data.isEvaluate === 0">待评价</text>
				<text v-if="data.status == 3 && data.isEvaluate === 1">已评价</text>
				<text v-if="data.status == 4 && data.isAffter === 0">退款中</text>
				<text v-if="data.status == 4 && data.isAffter === 1">已退款</text>
				<text v-if="data.status == 5">已完成</text>
			</view>
		</OrderGoodsCard>
		<!-- 待付款 -->
		<view class="operate" v-if="data.status == 0">
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="cancelOrderNow()">取消订单
			</u-button>
			<u-button type="gold" size="small" shape="circle" @click="goPay()">去付款</u-button>
		</view>
		<!-- 待发货 -->
		<view class="operate" v-if="data.status == 1">
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="showRemindAgain('提醒')"
				v-if="data.isRemind === 1">提醒发货</u-button>
			<u-button type="gold" size="small" shape="circle" @click="pushDeliveryNow()"
				v-if="data.isRemind === 0">提醒发货</u-button>
		</view>
		<!-- 待收货 -->
		<view class="operate" v-if="data.status == 2">
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="applyAfterSales()">申请售后
			</u-button>
			<u-button type="gold" class="operate_item" size="small" shape="circle" @click="goExpress()">查看物流</u-button>
			<u-button type="gold" size="small" shape="circle" @click="confirmReceiptNow()">确认收货
			</u-button>
		</view>
		<!-- 待评价 -->
		<view class="operate" v-if="data.status == 3">
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="deleteOrderNow()">删除记录
			</u-button>
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="applyAfterSales()">申请售后
			</u-button>
			<u-button type="default" size="small" shape="circle" @click="showRemindAgain('评价')"
				v-if="data.isEvaluate === 1">去评价</u-button>
			<u-button type="gold" size="small" shape="circle" @click="goEvaluate()"
				v-if="data.isEvaluate === 0">去评价</u-button>
		</view>
		<!-- 待评价 -->
		<view class="operate" v-if="data.status == 4">
			<u-button type="default" class="operate_item" size="small" shape="circle" @click="deleteOrderNow()">删除记录
			</u-button>
			<u-button type="gold" size="small" shape="circle" @click="goOrderInfo()">查看详情
			</u-button>
		</view>
		<!-- 已完成 -->
