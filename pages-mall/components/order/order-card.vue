	...
	<view class="slot" @click.stop="goOrderInfo">
		...
		<OrderGoodsCard :data="data" showBorderBottom>
			<view class="status-slot">
				...
				<text v-if="data.status == 4">退款</text>
		...
		<view class="operate" v-if="data.status == 1">
			<u-button type="default" size="small" shape="circle" @click="showRemindAgain()" v-if="data.isRemind === 1">提醒发货</u-button>
		...
		<view class="operate" v-if="data.status == 2">
			...
			<u-button type="gold" size="small" shape="circle" @click="confirmReceipt()">确认收货</u-button>
		</view>
		<!-- 待评价 -->
		<view class="operate" v-if="data.status == 3">
			<u-button type="default" style="margin-right: 24rpx;" size="small" shape="circle" @click="applyAfterSales()">申请售后</u-button>
			<u-button type="gold" size="small" shape="circle" @click="goEvaluate()">去评价</u-button>
		</view>
		<!-- 待评价 -->
		...
		cancelOrderNow() {
			...
			uni.showModal({
				...
				success: function(res) {
					if (res.confirm) {
						that.deleteOrder(that.data.user_id, that.data.order_id);
					}
				}
			});
		},
