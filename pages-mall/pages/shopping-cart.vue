    ...
    <u-navbar back-icon-name="arrow-leftward" title="购物车">
      <view class="slot-wrap">
        <u-button
          ...
          :ripple="true"
          @click="changeEdit"
          >{{ deleteShow === false ? "编辑" : "完成" }}</u-button
        >
      </view>
    </u-navbar>
      ...
      <view class="inner">
        <view class="check">
          <u-checkbox
            ...
            active-color="red"
            icon-size="16"
            v-model="checkAllStauts"
            @change="checkAllGoods"
          ></u-checkbox>
          <text>全选</text>
        </view>
        <view class="right">
          ...
          <u-button type="error" @click="goPayResult"
...
export default {
  ...
  computed: {
    totalPrice() {
      ...
      this.goodsList.map((item) =>
        item.checked ? (sum += Number(item.price) * item.num) : null
      );
      return sum;
    },
    checkAllStauts() {
  ...
  onShow() {
    const that = this;
    this.$u.api.getUserShoppingcart().then((res) => {
