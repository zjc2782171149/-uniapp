<template>
	<view class="page">
		...
		<view class="discounts">
			<TitleOperate title="优惠卷兑换 (有效期90天)" padding="28rpx 24rpx"></TitleOperate>
			<view class="list">
				<PointsDiscountsCard :data="item" v-for="(item, index) in discountsData" :key="index"
					@exchangeDiscount="exchangeDiscount"></PointsDiscountsCard>
			</view>
		</view>
	</view>
</template>
					...
					success: function(res) {
						if (res.confirm) {
							...
							if (that.userInfo.point >= item.points) {
								let task1 = that.$u.api.setCoupon({
								...
								let task2 = that.$u.api.updateUserPoint({
									user_id: that.userInfo.user_id,
